// Package "issue271" provides primitives to interact with the AsyncAPI specification.
//
...
type AppSubscriber interface {
	// Event subscribes to messages placed on the 'channel' channel
	Event(ctx context.Context, msg ChannelMessage) error
}
...
func addAppContextValues(ctx context.Context, path string) context.Context {
	ctx = context.WithValue(ctx, extensions.ContextKeyIsVersion, "0.0.2")
...
func (c *AppController) SubscribeAll(ctx context.Context, as AppSubscriber) error {
	...
	if err := c.SubscribeEvent(ctx, as.Event); err != nil {
		return err
	}
...
func (c *AppController) UnsubscribeAll(ctx context.Context) {
	c.UnsubscribeEvent(ctx)
}

// SubscribeEvent will subscribe to new messages from 'channel' channel.
//
// Callback function 'fn' will be called each time a new message is received.
func (c *AppController) SubscribeEvent(
	ctx context.Context,
	fn func(ctx context.Context, msg ChannelMessage) error,
) error {
	// Get channel path
	path := "channel"
	...
	go func() {
		for {
			...
			stop, err := c.listenToEventNextMessage(path, sub, fn)
			if err != nil {
				c.logger.Error(ctx, err.Error())
			}
...
func (c *AppController) listenToEventNextMessage(
	path string,
	...
	if err := c.executeMiddlewares(msgCtx, &acknowledgeableBrokerMessage.BrokerMessage, func(middlewareCtx context.Context) error {
		// Process message
		msg, err := brokerMessageToChannelMessage(acknowledgeableBrokerMessage.BrokerMessage)
		if err != nil {
			return err
		}
...
// UnsubscribeEvent will unsubscribe messages from 'channel' channel.
// A timeout can be set in context to avoid blocking operation, if needed.
func (c *AppController) UnsubscribeEvent(ctx context.Context) {
	// Get channel path
	path := "channel"
...
// PublishEvent will publish messages to 'channel' channel
func (c *UserController) PublishEvent(
	ctx context.Context,
	msg ChannelMessage,
) error {
	// Get channel path
...
// ChannelMessage is the message expected for 'ChannelMessage' channel.
...
// brokerMessageToChannelMessage will fill a new ChannelMessage with data from generic broker message
func brokerMessageToChannelMessage(bMsg extensions.BrokerMessage) (ChannelMessage, error) {
	var msg ChannelMessage
...
// toBrokerMessage will generate a generic broker message from ChannelMessage data
func (msg ChannelMessage) toBrokerMessage() (extensions.BrokerMessage, error) {
...
// PayloadSchema is a schema from the AsyncAPI specification required in messages
type PayloadSchema struct{}

const (
	// ChannelPath is the constant representing the 'Channel' channel path.
	ChannelPath = "channel"
)
